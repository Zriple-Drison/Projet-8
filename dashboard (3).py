...
def create_feature_importance_chart(client_data: dict) -> go.Figure:
    ...
    key_features = {
        ...
        "delay_rate": ("Taux retards paiements", -1, True),  # higher delay = worse
    ...
    for feat, (label, direction, positive_good) in key_features.items():
        if feat in client_data:
            ...
            if feat == "LOG_INCOME":
                ...
                normalized = min(1.0, val * 10)  # positive = bad
            ...
            is_good = (normalized * direction) > 0
            colors.append("#2E7D32" if is_good else "#C62828")
